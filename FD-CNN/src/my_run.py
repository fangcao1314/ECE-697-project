...
def shift_input(data,shift_value):
    return_data=np.zeros(data.shape)
    return_data[:,shift_value:]=data[:,shift_value:]
    return return_data

def add_noise(data,noise_std):
    noise=np.random.normal(0,noise_std,data.shape[0]*data.shape[1])
    noise=np.floor(noise)
    noise=np.reshape(noise, data.shape)
    return_data=np.zeros(data.shape)
    return_data=data+noise
    return_data=np.clip(return_data,0,255)
    return return_data
...
with tf.Session() as sess:
    ...
    p_y = np.argmax(sess.run(y_,feed_dict={x: add_noise(shift_input(test_x,shift_value),noise_std),keep_prob: 1.0}),1)
    print("准确率为 %f" % accuracy.eval(feed_dict={x: add_noise(shift_input(test_x,shift_value),noise_std), y: test_y, keep_prob: 1.0}))
